...
from ductor_slack.multiagent.supervisor import (
...
class TestStartupFailures:
    ...
    async def test_internal_api_start_failure_is_propagated(
        ...
        with (
            patch(
                "ductor_slack.multiagent.internal_api.InternalAgentAPI.start",
                new_callable=AsyncMock,
                return_value=False,
            ),
            patch("ductor_slack.multiagent.supervisor.AgentStack.create", new_callable=AsyncMock),
...
class TestStartSubAgent:
    ...
    async def test_handles_creation_failure(self, supervisor: AgentSupervisor) -> None:
        ...
        with patch(
            "ductor_slack.multiagent.supervisor.AgentStack.create",
...
class TestStopAll:
    ...
    async def test_cancellation_unblocks_start(self, supervisor: AgentSupervisor) -> None:
        ...
        with (
            ...
            patch(
                "ductor_slack.multiagent.shared_knowledge.SharedKnowledgeSync",
            ) as mock_sks_cls,
        ):
            mock_sks = MagicMock()
