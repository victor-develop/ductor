...
class TestTransportRegistry:
    ...
    def test_telegram_transport(self) -> None:
        ...
        config.is_multi_transport = False
        fake_bot = MagicMock()
        with patch("ductor_slack.messenger.telegram.app.TelegramBot", return_value=fake_bot):
    ...
    def test_matrix_transport(self) -> None:
        ...
        config.is_multi_transport = False
        fake_bot = MagicMock()
        with patch("ductor_slack.messenger.matrix.bot.MatrixBot", return_value=fake_bot):
            bot = create_bot(config, agent_name="test")
        assert bot is fake_bot
    ...
    def test_slack_transport(self) -> None:
        ...
        config.is_multi_transport = False
        fake_bot = MagicMock()
        with patch("ductor_slack.messenger.slack.bot.SlackBot", return_value=fake_bot):
            bot = create_bot(config, agent_name="test")
        assert bot is fake_bot
