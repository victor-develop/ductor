...
_LIFECYCLE = "ductor_slack.cli_commands.lifecycle"
_STATUS = "ductor_slack.cli_commands.status"
_SERVICE = "ductor_slack.cli_commands.service"
...
class TestLoadConfig:
    ...
    def test_creates_config_from_example(self, tmp_path: Path) -> None:
        from ductor_slack.__main__ import load_config
...
class TestRunTelegram:
    async def test_exits_on_missing_token(self, tmp_path: Path) -> None:
        from ductor_slack.__main__ import run_telegram
    ...
    async def test_runs_bot_with_valid_config(self, tmp_path: Path) -> None:
        ...
        with (
            patch("ductor_slack.__main__.resolve_paths"),
            patch("ductor_slack.infra.pidlock.acquire_lock"),
            patch("ductor_slack.infra.pidlock.release_lock"),
            patch(
                "ductor_slack.multiagent.supervisor.AgentSupervisor",
...
class TestIsConfiguredExtended:
    ...
    def test_configured_with_slack_bot_allowlist(self, tmp_path: Path) -> None:
        from ductor_slack.__main__ import _is_configured
...
class TestStopBot:
    def test_stop_kills_running_process(self, tmp_path: Path) -> None:
        from ductor_slack.cli_commands.lifecycle import stop_bot
        ...
        with (
            ...
            patch("ductor_slack.infra.pidlock._is_process_alive", return_value=True),
            patch("ductor_slack.infra.pidlock._kill_and_wait") as mock_kill,
        ):
            stop_bot()
...
class TestUpgradeCli:
    def test_upgrade_with_pipx(self, tmp_path: Path) -> None:
        from ductor_slack.cli_commands.lifecycle import upgrade

        paths = _make_paths(tmp_path)
        paths.ductor_home.mkdir(parents=True)
        with (
            patch("ductor_slack.infra.install.detect_install_mode", return_value="pipx"),
            patch(f"{_LIFECYCLE}.resolve_paths", return_value=paths),
            patch(
                "ductor_slack.infra.updater.perform_upgrade_pipeline",
    ...
    def test_upgrade_with_pip(self, tmp_path: Path) -> None:
        ...
        paths.ductor_home.mkdir(parents=True)
        with (
            patch("ductor_slack.infra.install.detect_install_mode", return_value="pip"),
            patch(f"{_LIFECYCLE}.resolve_paths", return_value=paths),
            patch(
                "ductor_slack.infra.updater.perform_upgrade_pipeline",
    ...
    def test_upgrade_rejects_dev_mode(self) -> None:
        ...
        with (
            patch("ductor_slack.infra.install.detect_install_mode", return_value="dev"),
...
class TestReExecBot:
    def test_re_exec_uses_popen_on_posix(self) -> None:
        from ductor_slack.cli_commands.lifecycle import _re_exec_bot
        ...
        with (
            ...
            _re_exec_bot()
        mock_popen.assert_called_once_with([sys.executable, "-m", "ductor_slack"])
...
class TestStartBotRestart:
    ...
    def test_exit42_with_supervisor_exits(self) -> None:
        from ductor_slack.cli_commands.lifecycle import start_bot

        with (
            patch(f"{_LIFECYCLE}.resolve_paths"),
            patch("ductor_slack.logging_config.setup_logging"),
            patch("ductor_slack.__main__.load_config", return_value=self._mock_config()),
...
class TestCountLogErrors:
    def test_counts_errors_in_log(self, tmp_path: Path) -> None:
        from ductor_slack.cli_commands.status import count_log_errors
...
class TestUninstall:
    def test_uninstall_removes_workspace(self, tmp_path: Path) -> None:
        from ductor_slack.cli_commands.lifecycle import uninstall
...
class TestMainDispatch:
    def test_help_command(self) -> None:
        from ductor_slack.__main__ import main

        with (
            patch("sys.argv", ["ductor", "help"]),
            patch("ductor_slack.__main__._print_usage") as mock_usage,
        ):
            main()
    ...
    def test_status_command(self) -> None:
        ...
        with (
            ...
            patch("ductor_slack.__main__._cmd_status") as mock_status,
        ):
            main()
    ...
    def test_stop_command(self) -> None:
        ...
        with (
            ...
            patch("ductor_slack.__main__._stop_bot") as mock_stop,
        ):
            main()
    ...
    def test_default_starts_bot_when_configured(self) -> None:
        ...
        with (
            ...
            patch("ductor_slack.__main__._is_configured", return_value=True),
            patch("ductor_slack.__main__._start_bot") as mock_start,
        ):
            main()
    ...
    def test_default_runs_onboarding_when_unconfigured(self) -> None:
        ...
        with (
            patch("sys.argv", ["ductor"]),
            patch("ductor_slack.__main__._is_configured", return_value=False),
            patch(
                "ductor_slack.cli.init_wizard.run_onboarding", return_value=False
            ) as mock_onboard,
            patch("ductor_slack.__main__._start_bot") as mock_start,
        ):
            main()
    ...
    def test_default_does_not_start_bot_when_service_installed(self) -> None:
        ...
        with (
            patch("sys.argv", ["ductor"]),
            patch("ductor_slack.__main__._is_configured", return_value=False),
            patch("ductor_slack.cli.init_wizard.run_onboarding", return_value=True),
            patch("ductor_slack.__main__._start_bot") as mock_start,
        ):
            main()
    ...
    def test_upgrade_command(self) -> None:
        ...
        with (
            ...
            patch("ductor_slack.__main__._upgrade") as mock_upgrade,
        ):
            main()
    ...
    def test_onboarding_command(self) -> None:
        ...
        with (
            ...
            patch("ductor_slack.__main__._cmd_setup") as mock_setup,
        ):
            main()
...
class TestSetupCommand:
    def test_setup_starts_bot_when_service_not_installed(self) -> None:
        from ductor_slack.__main__ import _cmd_setup

        with (
            patch("ductor_slack.__main__._stop_bot"),
            patch("ductor_slack.__main__.resolve_paths"),
            patch("ductor_slack.__main__._is_configured", return_value=False),
            patch("ductor_slack.cli.init_wizard.run_onboarding", return_value=False),
            patch("ductor_slack.__main__._start_bot") as mock_start,
        ):
            _cmd_setup(False)
        mock_start.assert_called_once_with(False)
...
class TestMainHelpers:
    def test_parse_service_subcommand_ignores_flags(self) -> None:
        from ductor_slack.cli_commands.service import _parse_service_subcommand
    ...
    def test_cmd_service_without_subcommand_prints_help(self) -> None:
        from ductor_slack.cli_commands.service import cmd_service
    ...
    def test_cmd_service_install_dispatches_backend(self) -> None:
        ...
        with patch("ductor_slack.infra.service.install_service") as mock_install:
    ...
    def test_cmd_service_status_dispatches_backend(self) -> None:
        ...
        with patch("ductor_slack.infra.service.print_service_status") as mock_status:
            cmd_service(["service", "status"])
        mock_status.assert_called_once()

    def test_print_usage_calls_status_when_configured(self) -> None:
        from ductor_slack.cli_commands.status import print_usage
    ...
    def test_print_usage_shows_not_configured_panel(self) -> None:
        ...
        with patch("ductor_slack.__main__._is_configured", return_value=False):
            print_usage()

    def test_cmd_status_prints_not_configured(self) -> None:
        from ductor_slack.__main__ import _cmd_status

        with patch("ductor_slack.__main__._is_configured", return_value=False):
            _cmd_status()

    def test_cmd_restart_stops_and_reexecs(self) -> None:
        from ductor_slack.cli_commands.lifecycle import cmd_restart
    ...
    def test_default_action_configured_starts(self) -> None:
        from ductor_slack.__main__ import _default_action
