...
from ductor_slack.messenger.matrix.credentials import login_or_restore
from ductor_slack.messenger.matrix.id_map import MatrixIdMap
from ductor_slack.messenger.matrix.sender import send_rich as matrix_send_rich
from ductor_slack.messenger.matrix.streaming import MatrixStreamEditor
from ductor_slack.messenger.matrix.typing import MatrixTypingContext
...
class MatrixBot:
    ...
    async def run(self) -> int:
        ...
        from ductor_slack.messenger.matrix.startup import run_matrix_startup
    ...
    async def _on_media(self, room: MatrixRoom | object, event: RoomMessageMedia | object) -> None:
        ...
        from ductor_slack.messenger.matrix.media import resolve_matrix_media
    ...
    async def _send_selector_response(
        ...
        from ductor_slack.messenger.matrix.buttons import REACTION_DIGITS
        from ductor_slack.orchestrator.selectors.models import ButtonGrid
    ...
    async def _handle_button_callback(
        ...
        from ductor_slack.messenger.callback_router import route_callback
    ...
    async def _handle_upgrade_callback(self, room_id: str, data: str) -> None:
        ...
        if data.startswith("upg:cl:"):
            ...
            if not version:
                return
            from ductor_slack.infra.version import fetch_changelog
    ...
    async def _handle_ns_callback(self, room_id: str, key: SessionKey, data: str) -> None:
        ...
        from ductor_slack.messenger.telegram.callbacks import parse_ns_callback
