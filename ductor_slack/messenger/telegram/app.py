...
from ductor_slack.commands import BOT_COMMANDS as _COMMAND_DEFS
from ductor_slack.commands import MULTIAGENT_SUB_COMMANDS as _MA_SUB_DEFS
...
from ductor_slack.infra.restart import EXIT_RESTART, consume_restart_marker
from ductor_slack.infra.updater import UpdateObserver
from ductor_slack.infra.version import VersionInfo, get_current_version
from ductor_slack.log_context import set_log_context
from ductor_slack.messenger.notifications import NotificationService
from ductor_slack.messenger.telegram.callbacks import (
...
from ductor_slack.messenger.telegram.formatting import markdown_to_telegram_html
from ductor_slack.messenger.telegram.handlers import (
    handle_abort,
    handle_abort_all,
    handle_command,
    handle_interrupt,
...
from ductor_slack.messenger.telegram.media import (
    has_media,
...
from ductor_slack.messenger.telegram.middleware import (
    MQ_PREFIX,
    AuthMiddleware,
    SequentialMiddleware,
)
from ductor_slack.messenger.telegram.sender import SendRichOpts, send_rich
from ductor_slack.messenger.telegram.sender import (
...
from ductor_slack.messenger.telegram.typing import TypingContext as _TypingContext
...
def _rebuild_commands() -> None:
    ...
    from ductor_slack.commands import get_bot_commands, get_multiagent_sub_commands
...
class TelegramBot:
    ...
    async def _on_startup(self) -> None:
        from ductor_slack.messenger.telegram.startup import run_startup
    ...
    async def _send_join_notification(self, chat_id: int) -> None:
        ...
        if not text:
            return
        from ductor_slack.messenger.telegram.sender import _send_text_chunks
    ...
    async def _show_welcome(self, message: Message) -> None:
        ...
        from ductor_slack.cli.auth import check_all_auth
    ...
    async def _on_forum_topic_created(self, message: Message) -> None:
        ...
        from ductor_slack.messenger.telegram.topic import get_topic_name_from_message
    ...
    async def _on_session(self, message: Message) -> None:
        ...
        try:
            if session_followup:
                ...
                from ductor_slack.orchestrator.core import NamedSessionRequest
    ...
    async def _on_restart(self, message: Message) -> None:
        if self._config.group_mention_only and not self._is_addressed(message):
            return
        from ductor_slack.infra.restart import write_restart_sentinel
    ...
    async def _route_special_callback(
        ...
        from ductor_slack.orchestrator.selectors.model_selector import is_model_selector_callback

        if is_model_selector_callback(data):
            await self._handle_model_selector(key, message_id, data)
            return True

        from ductor_slack.orchestrator.selectors.cron_selector import is_cron_selector_callback
    ...
    async def _route_prefix_callback(
        ...
        from ductor_slack.orchestrator.selectors.session_selector import (
            is_session_selector_callback,
        )
        from ductor_slack.orchestrator.selectors.task_selector import is_task_selector_callback
    ...
    async def _handle_model_selector(self, key: SessionKey, message_id: int, data: str) -> None:
        ...
        from ductor_slack.orchestrator.selectors.model_selector import handle_model_callback
    ...
    async def _handle_cron_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from ductor_slack.orchestrator.selectors.cron_selector import handle_cron_callback
    ...
    async def _handle_session_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from ductor_slack.orchestrator.selectors.session_selector import handle_session_callback
    ...
    async def _handle_task_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from ductor_slack.orchestrator.selectors.task_selector import handle_task_callback
    ...
    async def _handle_ns_callback(
        ...
        async with self._sequential.get_lock(key.lock_key):
            if self._config.streaming.enabled:
                from ductor_slack.orchestrator.flows import named_session_streaming

                result = await named_session_streaming(self._orch, key, session_name, label)
            else:
                from ductor_slack.orchestrator.flows import named_session_flow
    ...
    async def _handle_webhook_wake(self, chat_id: int, prompt: str) -> str | None:
        ...
        from ductor_slack.bus.envelope import LockMode
        ...
        from ductor_slack.bus.adapters import from_webhook_wake
    ...
    async def _on_update_available(self, info: VersionInfo) -> None:
        ...
        from ductor_slack.messenger.telegram.upgrade_handler import on_update_available
    ...
    async def _handle_upgrade_callback(
        ...
        from ductor_slack.messenger.telegram.upgrade_handler import handle_upgrade_callback
