...
from ductor_slack.app_identity import CLI_COMMAND
...
def kill_all_ductor_processes() -> int:
    ...
    2. PowerShell scan for ``python.exe``/``pythonw.exe`` running from the
       pipx venv directory (covers ``pythonw.exe -m ductor_slack``)
...
def _kill_venv_python_windows(current_pid: int) -> int:
    ...
    ``pythonw.exe -m ductor_slack`` inside ``~\pipx\venvs\ductor\``.
